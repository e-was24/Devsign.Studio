...
export default function Post({ onJourneyAdded }) {
  ...
  const [indexLoading, setIndexLoading] = useState(false); // loading khusus fetch index
  ...
  const fetchNextOrderIndex = async () => {
    ...
    try {
      ...
      console.error(err);
      // kalau gagal fetch, biarin default 1 aja, ga usah blokir user
    } finally {
      setIndexLoading(false);
    }
  };

  // --- Dipanggil saat tombol tambah journey diklik ---
  const handleOpen = () => {
    setIsOpen(true);
    fetchNextOrderIndex();
  };
  ...
  return (
    <>
      <button className="btn-post" onClick={handleOpen}>
